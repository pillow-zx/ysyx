...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)             \
    ...
    // INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw, R, R(rd) = SEXT((sword_t)src1 / (sword_t)src2, 32)); // divw: rd = rs1 / rs2 (signed, 32-bit op) [RV64I]
    ...
    // I型指令
    /// 控制状态寄存器指令, CSR指令, 这些指令用于读写RISC-V处理器中的控制状态寄存器, 未实现csrrwi, csrrsi, csrrci指令
    INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, I, word_t t = CR(imm); CW(imm, src1); R(rd) = t);             // csrrw: rd = CSR[imm]; CSR[imm] = rs1

                ...
                switch (rules[i].token_type) {
                    ...
                    case TK_N_EQ:
                        ...
                        if (tokens[nr_token].type == '*' || tokens[nr_token].type == '-') {
                            if (nr_token == 0 || tokens[nr_token - 1].type == 0 || tokens[nr_token - 1].type == TK_NEGTIVE || tokens[nr_token - 1].type == '(' ||
                                tokens[nr_token - 1].type == '+' || tokens[nr_token - 1].type == '-' || tokens[nr_token - 1].type == TK_EQ || tokens[nr_token - 1].type == TK_AND ||
                                tokens[nr_token - 1].type == TK_N_EQ) {
                                tokens[nr_token].type = tokens[nr_token].type == '*' ? TK_POINT : TK_NEGTIVE;
                            }
                        }
                        nr_token++;
                        break;
...
static int priority(int operator) {
    switch (operator) {
        ...
        case TK_EQ:
        case TK_N_EQ: return 1;
        case '+':
        case '-': return 2;
        case '*':
        case '/': return 3;
        case TK_POINT:
        case TK_NEGTIVE: return 4;
        default: return 5;
    }
}
...
word_t eval(int p, int q) {
    if (p > q) {
        ...
        switch (tokens[position].type) {
            case TK_EQ: return val2 == val1;
            case TK_AND: return val2 && val1;
            case TK_N_EQ: return val2 != val1;

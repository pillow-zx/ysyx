...
void init_regex() {
    ...
    for (i = 0; i < NR_REGEX; i++) {
        // REG_EXTENDED 是一个宏定义，其值为
        // 1。如果设置了这个位，则使用扩展正则表达式语法；如果未设置，则使用基本正则表达式语法
        /* regcomp 函数用于编译正则表达式，并将结果存储在 regex_t
         * 类型的结构中，以便在后续的正则表达式匹配操作中使用。 该函数遵循 POSIX
         * 标准，参数包括目标结构指针 __preg、正则表达式模式字符串 __pattern 和编译标志 __cflags
         */
...
__attribute__((used)) = {};                // tokens数组用于按顺序存放已经被识别出的token信息
...
static bool make_token(char* e) {
    ...
    /* regmatch_t 结构体包含两个int类型成员 rm_so 和
     * rm_eo，分别表示匹配到的子串在原字符串中的起始和结束位置 */
    regmatch_t pmatch;
    ...
    while (e[position] != '\0') {
        ...
        for (i = 0; i < NR_REGEX; i++) {
            ...
            // 指向已编译的正则表达式的指针 要匹配的字符串 可存储的最大匹配数 存储匹配结果的数组
            // 匹配标志
            if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
                char* substr_start = e + position; // substr_start 是一个指向字符的指针，用于在 C
                // 语言中执行正则表达式匹配操作时，指向匹配结果的起始位置。
                int substr_len =
                    pmatch.rm_eo; // substr_len 是一个整数变量，用于存储子字符串的长度。
                ...
                if (e[position - 1] == '$') {
                    int j = position;
                    tokens[nr_token].str[j - position] = '$';
                    while ((e[j] >= 'a' && e[j] <= 'z') || (e[j] >= '1' && e[j] <= '9')) {
                        tokens[nr_token].str[j - position + 1] = e[j];
                        j++;
                    }
                    tokens[nr_token].str[j - position + 1] = '\0'; // 添加字符串结束符
                    position = position + j - position;
                }
                else if (e[position - 1] == '0' && (e[position] == 'x' || e[position] == 'X')) {
                    // strncpy(tokens[nr_token].str, e + position, 2);
                    // int j = position + 1;
                    // while (isxdigit(e[j])) {
                    //     tokens[nr_token + 1].str[j - position - 1] = e[j];
                    //     j++;
                    // }
                    // tokens[nr_token + 1].str[j - position] = '\0'; // 添加字符串结束符
                    // tokens[nr_token].str[2] = '\0'; // 添加字符串结束符
                    // position = position + j - position;
                }
                else {
...
// 将操作数压入栈
void apply_operator(Stack* num, OpStack* op) {
    word_t b;
    word_t a;
    word_t temp;
    ...
    case '/':
        ...
        break;
    case '!':
        temp = num->value[num->top--];
        apply_operator(num, op);
        if (num->value[num->top] != temp) {
            num->value[num->top] = 1;
        }
        else {
            num->value[num->top] = 0;
...
word_t calculate_expression() {
    ...
    for (int i = 0; tokens[i].str[0]; i++) {
        ...
        if (isdigit(tokens[i].str[0])) {
            // 如果是数字，将其压入操作数栈
            word_t value = 0;
            if (tokens[i].str[0] == 'x' || tokens[i].str[0] == 'X') {
                i += 2;
                while (isxdigit(tokens[i].str[0])) {
                    if (isdigit(tokens[i].str[0])) {
                        value = value * 16 + (tokens[i].str[0] - '0');
                    }
                    else if (tokens[i].str[0] >= 'a' && tokens[i].str[0] <= 'f') {
                        value = value * 16 + (tokens[i].str[0] - 'a' + 10);
                    }
                    else if (tokens[i].str[0] >= 'A' && tokens[i].str[0] <= 'F') {
                        value = value * 16 + (tokens[i].str[0] - 'A' + 10);
                    }
            ...
            while (tokens[i].str[j] != '\0') {
                temp[j] = tokens[i].str[j + 1];
                j++;
            }
            printf("%s\n", temp);
            bool success = false;
            num.value[++num.top] = isa_reg_str2val(temp, &success);
            if (success == false) {
                printf("Error: Invalid register name\n");
                return 0;
            }
            printf("num.value[%d] = %d\n", num.top, num.value[num.top]);
        }
        else if (tokens[i].str[0] == '(') {
            // 如果是左括号，将其压入操作符栈
            ...
            if ((tokens[i].str[0] == '!' || tokens[i].str[0]) && tokens[i + 1].str[0] == '=') {
                op.op[++op.top] = tokens[i].str[0];
                i++;
                continue;
            }
            else if (tokens[i].str[0] == '&' && tokens[i + 1].str[0] == '&') {
                op.op[++op.top] = tokens[i].str[0];
                i++;
                continue;
            }
            else if (tokens[i].str[0] == '*' && is_operator(tokens[i - 1].str[0])) {
                continue;
            }
            else {
                // 如果是运算符

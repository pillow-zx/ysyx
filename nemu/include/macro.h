...
#define IFDEF(macro, ...)                                                                                              \
    MUXDEF(macro, __KEEP, __IGNORE)(__VA_ARGS__) // 如果宏 macro 已定义，则保留并处理 ... 中的代码，否则忽略这些代码。
// keep the code if a boolean macro is undefined
#define IFNDEF(macro, ...)                                                                                             \
    MUXNDEF(macro, __KEEP, __IGNORE)(__VA_ARGS__) // 如果宏 macro 未定义，则保留并处理 ... 中的代码，否则忽略这些代码。
...
// 符号拓展宏,将一个整数 x 的低 len 位符号拓展为 64 位无符号整数
// 其中x代表要进行符号拓展的整数，len代表要进行符号拓展的位数。
#define SEXT(x, len)                                                                                                   \
    ({                                                                                                                 \
        struct {                                                                                                       \
            int64_t n : len;                                                                                           \
